kbd15←{
     ⍝ takes optional number of keys as left argument and .klc filename as right argument
     ...
     row←where matrix[;1]Is Hex¨n⊃(43,⍨29+⍳11)(86,⍨43+⍳10) ⍝ the keys to rotate
     matrix←¯1⊖at row⊢matrix                               ⍝ rotate bottom to top and
     ...
     indicesX←where'X'=kbd ⍝ indices of placeholders for shifted keys
     indicesY←where'Y'=kbd ⍝ indices of placeholders for unshifted keys
     ...
     symbols←'@'⊣¨indices                                         ⍝ one  at  for each dead key
     ...
     symbols at indices⊢kbd ⍝ make all the substitutions
 }
